...
parser.add_argument('--ofdir', type=str, required=False,
                    default='GREW_pre/flow/test/gallery/',
...
parser.add_argument('--mode', type=str, required=False,
                    default='ft',
                    help="ft: gallery subset; test: probe subset")
...
folder_dir = 'tfimdb_grew_N06000_' + mode + '_of' + str(25) + '_64x64'
...
for id_ix in id_folders:
	...
	if im is not None:
		...
		pattern = id_ix.split('/')[-2]

		if mode == 'test':
			outpath = os.path.join(outdir, folder_dir, pattern + '.h5')
		else:
